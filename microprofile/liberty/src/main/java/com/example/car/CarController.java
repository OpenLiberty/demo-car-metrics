...
public class CarController {
	...
	@Path("startDrive")
	@Timed(name="startDriveTimer", absolute=true)											// 2 - add a Timer to track how long a method takes
	@GET
		...
		Thread t = new Thread(new Runnable() {
			public void run() {				
				while (speedThreadStarted.get()) {
					...
					float newSpeed = targetSpeed + ((targetSpeedVar > 0) ? (float) random.nextInt(targetSpeedVar) : 0.0f);
